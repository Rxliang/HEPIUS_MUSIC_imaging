...
if 0 % run on unconfigured system
  disp('*** Note: this debug mode requires c:\temp to exist and be writable');
  setenv('BNS_HOME', 'c:\temp');
  setenv('BNS_DATA', 'c:\temp');
  if ~exist('c:\temp\tmp', 'dir')
    mkdir('c:\temp\tmp');
  end
...
numSaveFramesPerBatch = 650; % 100 is 5 sec
...
PRIAcqInPlace_uSec = PRIAcq_uSec;
...
% for saving IQ:
xSVWidthIQ_mm = xSVWidth_mm*10;
zSVWidthIQ_mm = zSVWidth_mm*30;
...
% For spectrogram we, we need to ensure we don't have glitching,
...
% gen and display largeSD every N frame
largeSDParms.largeSDFrameInterval=2;
...
if phantomFlag
  ...
  xSVWidth_mm = 5;
  zSVWidth_mm = 3;
  PRIAcq_uSec = 244.5; % select so nPRI is even
  PRIAcqInPlace_uSec = PRIAcq_uSec;
  SDLarge.xSDWidth_mm = 18;
% default height of overlay
  SDLarge.zSDHeight_mm = 25; 
  SDLarge.zSDStart_mm = 32;
  SDLarge.zSDEnd_mm = SDLarge.zSDStart_mm+SDLarge.zSDHeight_mm;
...
evParms.state.initFigsDone = 0; % ultrasound system figs not
                                % initialized yet
evParms.state.recording = [0 0]; % do not start recording IQ, per channel
...
maxDepthForRcvBufferCalc_mm = 120;
...
evParms.gate.largeGateIQ = 0; % use the larger PData for SD, as
                              % specified by {xz}SVWidthIQ_mm

evParms.flag.BLineWT = 0; % B-line walltrack
evParms.BLineWT.numTxBWT=0;
evParms.flag.doWT = 0; % enable wall track via separate PDATA
% make separate bline transmit for m-mode
evParms.flag.BLineWT = 1;
evParms.flag.BLineWTCapability = 1;
...
if ~iseven(evParms.ev.nPRIs)
  error('Adjust PRIAcqInPlace_uSec so that nPRIs is an even number');
end
...
Trans.name = 'custom'; % allow any probe id (fix for vera 5 LT
                       % which does not show constant probe ID)
...
P(PIndB).startDepth = floor(startDepth_mm / lambda_mm);
endDepthPre = endDepth_mm / lambda_mm;
P(PIndB).endDepth = ceil(endDepthPre);
P(PIndB).maxDepth = maxDepth_mm*scaleToWvl;
...
P(PIndSD).startDepth = P(PIndB).startDepth;
P(PIndSD).endDepth = P(PIndB).endDepth;
...
if doCFI
  ...
  PIndCFI=2;
  P(PIndCFI).startDepth = SDLarge.zSDStart_mm/lambda_mm;
  P(PIndCFI).endDepth = SDLarge.zSDStart_mm/lambda_mm;
  P(PIndCFI).maxDepth = P(PIndB).maxDepth;
...
maxDistanceForRcvBufferCalc_mm = ...
    sqrt(maxDepthForRcvBufferCalc_mm.^2 + (FOV.maxHalfWid_mm*2).^2) ...
    - FOV.startDepth_mm;
                                      
numberOfWvlRoundTrip = (maxDistanceForRcvBufferCalc_mm/1e3)*2/lambda_m;
numberOfSamplesPerWvl = 4; % NS200BW for B mode
...
if multiAngleB
  ...
  % SD sequence
  uscmsg(execState);
  lastFrameTTNA = 0; % introduce longer delay at frame end, to see
...
evParms.ev.lastFrameTTNA = lastFrameTTNA;
...
UI(1).Control =  {'UserB7','Style','VsSlider','Label','Sens. Cutoff',...
                  'SliderMinMaxVal',[0,1.0,Recon(recon.reconIndB).senscutoff],...
...
UI(7).Callback = nicpseq_wbdCallback;
