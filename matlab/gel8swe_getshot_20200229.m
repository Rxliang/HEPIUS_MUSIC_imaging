...
useFreq_MHz = 8.9286; % 10.4167; % 12.5;

machPush = 1; numPush=3;
...
pushCycle  = 1000;     % number of cycles
maxVoltage = 30;

pushDuration_us = pushCycle / useFreq_MHz;

ttnaAcq_us = 100; % was 100
ttnaPush_us = 500; % pushDuration_us*2;  % 500;
ttnEB_us =  200000;
...
SWIROI.height = 40; % was 50;  % [width and height in wavelength]
...
Trans.name = 'GEL8-18i';
Trans.units = 'mm';
Trans = computeTrans_jon(Trans);  % L7-4 transducer is 'known' transducer so we can use computeTrans.
...
for q = 2:nTX
  ...
  TX(q).Apod(Trans.numelements/2-TX(q).pushElements/2 +1 : ...
             Trans.numelements/2+TX(q).pushElements/2) = 1;
