...
%P.endDepthMm = 15;    % end depth in mm
...
dop.saveData = 1;
dop.fname = 'MUSIC_Plane_Axial_WeakFlowHigh_Normal';
dop.cpl = 70;   
...
PData(2).PDelta = [Trans.spacing/2,0,dop.cycles/2];
...
ROISmall_mm = [-0.057 0.736   
                9.495 11.17];
dROISmall_mm = ROISmall_mm(:,2)-ROISmall_mm(:,1);


ROISmall = ROISmall_mm/lambda_mm;
dROISmall = dROISmall_mm/lambda_mm;

PDataIndSD = 3;
p = PDataIndSD;
PData(p).Coord = 'rectangular';
PData(p).PDelta = [Trans.spacing/2,0,dop.cycles/2];
PData(p).Size(1) = ceil(dROISmall(2)/PData(p).PDelta(3));
PData(p).Size(2) = ceil(dROISmall(1)/PData(p).PDelta(1));
PData(p).Size(3) = 1;
PData(p).Origin = [ROISmall(1,1), 0, ROISmall(2,1)];
PData(p).Region = computeRegions(PData(p));
...
%Resource.ImageBuffer(1).numFrames = 10; %JSM, can this work in replay, no
% benefit
...
Resource.RcvBuffer(2).numFrames = 10;     % 10 frames for RF data.

if simulateMode ==2
  Resource.RcvBuffer(1).rowsPerFrame = size(RcvData{1},1);
  Resource.RcvBuffer(1).numFrames = size(RcvData{1},3);
  Resource.RcvBuffer(2).rowsPerFrame = size(RcvData{2},1);
  Resource.RcvBuffer(2).numFrames = size(RcvData{2},3);
end
...
Resource.ImageBuffer(3).numFrames = 10; % jsm 1->10
...
ii=interIndSD;
pp=PDataIndSD;
Resource.InterBuffer(ii).numFrames = 10;  % one intermediate buffer needed.
Resource.InterBuffer(ii).pagesPerFrame = dop.PRI; 
Resource.InterBuffer(ii).rowsPerFrame = PData(pp).Size(1);
Resource.InterBuffer(ii).colsPerFrame = PData(pp).Size(2);
...
% needs to be Matlab fig for spectral dop
...
reconIndSD=4;
r=reconIndSD;
Recon(4).pdatanum = PDataIndSD;
Recon(4).rcvBufFrame = -1;
Recon(4).IntBufDest = [interIndSD, -1];
Recon(4).ImgBufDest = [];
end
...
ReconInfo = repmat(struct('mode', 'accumIQ', ...  % accumulate IQ data.
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 1), 1, P.numPlanes+(2*dop.numAngs*dop.PRI)+P.numPlanes);%
...
if 1
rc = Recon(3).RINums(end);

% SD Doppler
ReconInfo(rc+1).Pre = 'clearInterBuf';
ReconInfo(rc+1).mode = 'replaceIQ';
for k = 1:dop.PRI
    ReconInfo(rc+(k-1)*dop.numAngs+1).mode = 'replaceIQ';
    for j = 1:dop.numAngs  % For each row in the column
        ReconInfo(rc+(k-1)*dop.numAngs+j).txnum = lastBmodeTxArr1+j;
        ReconInfo(rc+(k-1)*dop.numAngs+j).rcvnum = lastBmodeRcvArr1+P.numPlanes*P.numAvgs+(k-1)*dop.numAngs+j;
        ReconInfo(rc+(k-1)*dop.numAngs+j).pagenum = k;
        ReconInfo(rc+(k-1)*dop.numAngs+j).regionnum = 1;
    end
end
rce = rc + dop.numAngs*dop.PRI;
Recon(reconIndSD).RINums = rc+1:rce;
end
...
procIndSD = 5;
Process(procIndSD).classname = 'External';
Process(procIndSD).method = 'spectralDoppler_JP';
Process(procIndSD).Parameters = {'srcbuffer','inter',...
                         'srcbufnum',4,...
                         'srcframenum',-1,...
                         'srcpagenum',0,...
                         'dstbuffer','none'};
...
for l = 1:Resource.RcvBuffer(2).numFrames
    ...
    %    Event(n).recon = [3,2];
    Event(n).recon = [4,3,2];    
    %    Event(n).process = [2,procIndSD];
    Event(n).process = [procIndSD];
    Event(n).seqControl = 0;
    n = n+1;
    
    Event(n).info = 'CFIProcJSM';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    %    Event(n).recon = [3,2];
    Event(n).recon = 0;    
