...
[psched ]   = spectralDoppler_4p9p2('restrictAcqPRF',framePeriodnominal_uSec,dopPRFnominal);
[pschedAll ]= spectralDoppler_4p9p2('restrictAcqPRF',framePeriodnominal_uSec,[]);
disp(' '),
...
sgp= spectralDoppler_4p9p2('spectrogramParameters',framePeriod,dopSweepTimeIndex);
sweepTime = sgp.THist;
...
Process(2).method = 'spectralDoppler_4p9p2';
...
function dopplerPRFCallback( hObject, ~, UIValue)
    ...
    psched = spectralDoppler_4p9p2('restrictAcqPRF',round(framePeriod*1e6),dopPRFnom);
    acqPRF = psched.PRFAcq ;
    dopPRF = psched.PRFDop ;
    ttna_microsec =   psched.PRIAcq_uSec ;
...
%-UI#5Callback - Sweep Time
    ...
    sgp=spectralDoppler_4p9p2('spectrogramParameters',TFrame,1);
