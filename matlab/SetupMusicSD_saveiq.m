...
filename = ['MatFiles/' mfile];
delete([filename '.mat'])

% when this is 1, starts up with simultaneous B/SD
BSDModeFlag = 1;
syncFlag=0;
transferDelayFlag=1;
dopRows = 20;
...
Trans.name = 'custom'; % prevent name mismatch complaint
...
Apod(elemRange)=1;                   %%%%%%%%%%%%%%%%% element index
...
for i = 1:nfrms
    ...
    for j = 1:2:maxPRIs
        ...
        Receive(k+j).Apod(elemRange) = 1.0;        %%%%%%%%%%%%%%%%%element index 65,128 129,192
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;
        if j==1, Receive(k+j).callMediaFunc = 1; end
        % -- Doppler.
        %        Receive(k+j+1).Apod(round((33:96)/2)) = 1.0; % start Dop aperture at element 33
        Receive(k+j+1).Apod(elemRange) = 1.0; % start Dop aperture at element 33 %%%%%%%%%%%%%%%%%%%%%%element index
        Receive(k+j+1).TGC = 2;
...
SeqControl(nsc).argument = 3000; % us
...
function rangeChangeCallback(hObject, ~, UIValue )
    ...
    %    clear spectralDoppler_4p9p2_saveiq
    Control = evalin('base','Control');
    Control.Command = 'update&Run';
