...
useMusic=1;
% set all tx apods to zero
zeroTxApod = 0;

% Default version info, overwritten by productionsettings.m
version = [];
version.name = '*** Not for clinical use ***';
version.number = '0.0';
...
% Some UI parameters
evParms = [];
evParms.flag.canSaveImage = 1; % add capability of saving B+SDLarge im to file, to seq
evParms.reconParms.senscutoff = 0.2
evParms.lock.ctrlLockSleepTime_s = 0.25;
evParms.lock.ctrlUpdate = 0;
evParms.flag.debugSeqSwitchMode=debugSeqSwitchMode;
evParms.flag.zeroTxApod = zeroTxApod;
...
forceRecord=0; % don't require semaphore to record, overridden by productionsettings
...
noWarn = 1;
Trans.frequency = demodfreqcheckfn(10.4, noWarn);
...
evParms.ev.transferTTNA_us = 46000;
evParms.ev.transferTTNA_us = 25000; % josh

% after last Doppler tx when regular CFI not switched on

evParms.ev.transferTTNA_postSD_us = 45000; % 60000;
...
evParms.default.SD.dopWFCutoffNorm = 0.6*(0.025*2); % JSM 0.6 for MUSIC
...
evParms.ev.watchdogFilenameIm = fullfile(tmpDirSub, 'watchdogTimestampFileIm.txt');
...
%maxPRFNominal_Hz = 4525*3; % max PRF for B+SD 
%maxPRFNominal_Hz = 10000; % max PRF for B+SD 
                           %maxPRFNominal_Hz = 10000; % max PRF for B+SD 
                          %maxPRFNominal_Hz = 8000; % max PRF for B+SD 
%maxPRFNominal_Hz = 2500; % max PRF for B+SD 
...
%PRIAcq_uSec = 150.5; % sequence TTNA jsm comment out 20240117
...
%evParms.flag.largeSD=1; % overwritten by productionsettings 
...
SDLarge.zSDStart_mm = startDepth_mm; % note, used for pdata even when no
...
largeSDParms.pwrThres = 0.01; % 0.01 josh

largeSDParms.cpl = 60;  % default color priority level
largeSDParms.maxPower=20; % default power to override B-mode, was 10 JSM
largeSDParms.perspwr=20; % default power Doppler persistence
...
largeSDParms.pwrThreshLimits = [0 1];
largeSDParms.perspwrLimits = [0 99];
largeSDParms.cplLimits = [0 255];  
% JSM change
SDLarge.zSDMinHeight_mm = 1;  % was 10
SDLarge.xSDMinWidth_mm = 1;  % was 10
...
% overlay
numCFIPRIs = 16; % 20 is min for weak flow wall filters % was 16 number of pulses per frame for CDI, CFIEst uses pulses 3:end-2, so need to add 4, for 20, 26 appears to work
%josh dop.PRF = 4*dop.maxV/(P.c*100)*Trans.frequency*10^6;

%dopPRFCFI = 2.5e+03;
dopMaxV = 4; %cm/s   % Maximum velocity
dopPRFCFI = 4*dopMaxV/(Resource.Parameters.speedOfSound*100)*Trans.frequency*10^6;

pwrThresCFI = 0.01; % % josh 0.01,  0.35; % jsm, 0.42; % orig
persfreqCFI = 20; % perslevel
perspwrCFI = 20;
evParms.CFI.processMethod = 'reduceSpeckle2';
evParms.CFI.compressFactor = 70;
evParms.CFI.threshold = 70;
evParms.CFI.wallFilter = 'WeakFlowMedHigh';
evParms.CFI.wallFilter = 'regression';
...
% 20240302: doCFISepAtStart=1 will change the skip between
% recvs from 2 (interleaved B and SD ) to 1 and mess this up.
...
shallowDepth=0; % useful for shrinking RF size
% delay HW before each frame, converted to noop units of 200 ns in seq
preFrameHWDelay_s = 0;
...
if sectorProbe
  ...
  regionOverlapFactor3ray=1.2; % PData region overlap when 3 wide
                             % beams are used
  regionOverlapFactor4ray=1.1; % PData region overlap when 4 wide
                             % beams are used
  regionExtensionFactor=0.95;   % Overall PData mag factor vs
                             % txd width    
end
...
evParms.state.recording = evParms.flag.recordSpec;
...
if useMusic
    if useMusic==3
        Trans.name = 'MUSIC3_Biobox64';
    else
        Trans.name = 'MUSIC_Biobox64';
    end
    
    if useMusic==4 
        % 112 contiguous element with adapter
        Trans.name = 'GEL8-18i_112';
        txUsedInd= findcentraln(64, 1:112);
    end
    
    if useMusic==5 
        Trans.name = 'MUSIC_Biobox64'; % this is same seq as MUSIC3_Biobox with just fewer channels
...
evParms.ev.VMin = 0; % Vantage minimum voltage allowed

evParms.FOV = FOV;

if use312 | use6s |  usem5_64 | useMusic
...
% do not show slider
evParms.flag.depthOffsetSliderFlag = 0;
...
%evParms.ev.nPRIs = floor(2*framePeriod_s/PRIAcqInPlace_uSec*1e6)/2; % changed 20240117 to:
% needs to be even so nDopPRIs is an integer
evParms.ev.nPRIs = 2*floor(framePeriod_s/PRIAcqInPlace_uSec*1e6/2);
...
if evParms.flag.BLineWT
  ...
  evParms.BLineWT.focus_mm = 8; % JSM for MUSIC string phantom
  ...
  evParms.BLineWT.MMaxDepth_mm = 30; % JSM
...
if doCFI
  if use312 
    ...
    dopNumRays = 4; % no. of rays in frame
      ...
      % for shorter transducers, 3 rays probably better
...
%Trans.name = 'MUSIC_Biobox64';
%Trans.name = 'MUSIC3_Biobox64'; % this has differrent numbers
...
if useMusic == 6
  Trans.name = 'GEL8-18i';
  txUsedInd = findcentraln(64, 1:168);
end


Trans = computeTrans2(Trans);  % computeTrans is used for known transducers.
...
if 0 & useMusic
    txIndex = 1:Trans.numelements; % use only central row
                                   %txIndex = 1:64;
    if usem5_64 | useMusic==5
      txUsedInd = 129:192; %1:64;
    end
...
evParms.ind.txUsedInd = txUsedInd;
...
if doCFI
  if usec15 | usem5 | use6s | useMusic
  ...
  if doCFISep
    ...
    P(PIndCFI).cpl = evParms.CFI.threshold;
  end
...
evParms.ind.PDataIndWT = PDataIndSDStart+evParms.gate.numSD+evParms.flag.largeSD;

evParms.wt.MStoreDuration_s = 40;

% do not start recording IQ, per channel
%evParms.state.recording = ...
%    zeros(1, evParms.gate.numSD+evParms.flag.largeSD+evParms.flag.doWT);
...
if doCFI
  ...
  P(PIndCFI).expandCFIToEdges.flag = 0; % was 1, 20240302, shrinks PData
...
switch BWMode
  case 'BS100BW'
    numberOfSamplesPerWvl = 2;
  case 'BS200BW'
    numberOfSamplesPerWvl = 4;
end
...
%EventAnalysisTool warns when these buffers don't have same length
samplesPerEventBSD = max(samplesPerEventB, samplesPerEventSD);
FOV_BSD_maxDist_wvl = max(FOV_B.maxDist_wvl, FOV_SD.maxDist_wvl);
FOV_B.maxDist_wvl = FOV_BSD_maxDist_wvl;
FOV_SD.maxDist_wvl = FOV_BSD_maxDist_wvl;

% maximum number of B PRIs
samplesPerFrameB = evParms.ev.nPRIs/2*samplesPerEventBSD; 


samplesPerFrameSDDuplex = evParms.ev.nPRIs/2*samplesPerEventBSD; % maximum number of B PRIs
samplesPerFrameSDOnly = evParms.ev.nPRIs*samplesPerEventBSD; % maximum number of B PRIs
...
samplesPerFrameCFI =  samplesPerEventBSD*BModeEventsPerFramePreCFI +...
...
if evParms.flag.largeSD
  ...
  % for this case, don't allow enlargement
  PDataIndVec(end) = evParms.gate.PDataIndSDLarge;  
end
...
% base image display on PData(PDataIndB), assuming we start image at zero depth,
% and that image is centered laterally
pdB = pdata2roifn(PData(PDataIndB), lambda_mm);
DwWidth = ceil(pdB.xExt/Resource.DisplayWindow(1).pdelta);
DwHeight = ceil(pdB.zExt/Resource.DisplayWindow(1).pdelta);
...
if useMusic
    % keep these at full size, but reduce use in apod
  numDelays = Trans.numelements;
end
...
if usec15 | usem5 | use6s | useMusic
  ...
  apodSet(txUsedInd)=1;
  apodSetNull(txUsedInd)=0;
  
  if zeroTxApod
    apodSetTx = apodSetNull; 
  else      
    apodSetTx = apodSet;
  end
  apodSetRx = apodSet;
...
if doCFI
  TXIndCFI = numTXB+1;
  % TX(TXIndCFI) is set by makecfipdatatxfn.m
  % TXIndCFI is first of m beams
...
if UTA260MUX < 3 % not needed for 256 channel system
...
[dum, indCentralEl] = findclosestinvec(Trans.ElementPos(txUsedInd,1), ...
                                       xDopplerBeam_mm);
...
if focussedDoppler
  ...
  if useMusic
    indCentralElL = findcentraln(1, txUsedInd);
    indCentralElR = findcentraln(1, txUsedInd);
  end  
     
  lft = indCentralElL - floor(evParms.P(PIndSD).numTx/2);
  if lft < 1, lft = 1; end;
...
%if useMusic
% tmp = TX(TXIndSD);
% tmp.Apod = tmp.Apod(txUsedInd);
% TX(TXIndSD).Delay = computeTXDelays();  
...
RcvProfile(evParms.ind.RcvProfileIndB).LnaGain = rcvParms.lnaGain_dB.B; % Profile used for B-mode imaging

if evParms.flag.BLineWT
  RcvProfile(evParms.ind.RcvProfileIndB).LnaGain = rcvParms.lnaGain_dB.BWT;
  RcvProfile(evParms.ind.RcvProfileIndB).LnaZinSel = rcvParms.lnaZinSel.BWT;
end


% Profile used for Doppler. Force high-Z state for best Doppler sensitivity
RcvProfile(evParms.ind.RcvProfileIndCFI).LnaGain = rcvParms.lnaGain_dB.CFI;
RcvProfile(evParms.ind.RcvProfileIndCFI).LnaZinSel = rcvParms.lnaZinSel.CFI; 

RcvProfile(evParms.ind.RcvProfileIndSD).LnaGain = rcvParms.lnaGain_dB.SD; % Profile used for Doppler
RcvProfile(evParms.ind.RcvProfileIndSD).LnaZinSel = rcvParms.lnaZinSel.SD;
...
disp(BWMode);
evParms.rcvParms.sampleMode.B = BWMode;
evParms.rcvParms.sampleMode.SD = BWMode;
...
for i = 1:numSD+evParms.flag.largeSD
  ...
  numSDReconPages = length(Recon(evParms.recon.reconIndSDStart+i-1).RINums);
  ...
  Resource.InterBuffer(interBufferIndSDStart+i-1).colsPerFrame = ...
      PData(PDataIndVec(i)).Size(2);
end

if evParms.flag.largeSD & (numSDReconPages < evParms.largeSDParms.endPRI)
  error('For largeSD recon, need enough interbuffer pages to accommodate RIs. Try reducing evParms.largeSDParms.priSkip');
end
...
s    evParms.ev.dopPRF=evParms.ev.acqPRF;
...
if doCFI
  ...
  if 0 % version for control of CDI
      ...
      UI(uiInd).Control = {slider.dopplerpwrthresh.pos,'Style','VsSlider',...
                          'Label','CDIDopPwrThres','SliderMinMaxVal',...
                          [0.0,1.0,evParms.P(PIndCFI).pwrThres],...
  ...
  % max was 0.2
  UI(uiInd).Control = {slider.dopplerpwrthresh.pos,'Style','VsSlider',...
                      'Label','SDDopPwrThres','SliderMinMaxVal', ...
                      [0.0,1.0, evParms.largeSDParms.pwrThres],...
