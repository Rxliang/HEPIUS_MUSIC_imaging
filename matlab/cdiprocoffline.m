function IQOut = cdiprocoffline(IQData);
% Notice: 
...
pwrThres = evalin('base','evParms.largeSDParms.pwrThres');
imgPID =  3; %must match script, doppler Imag process % strmatch('Image',pc,'exact') ;

DopState = evalin('base','evParms.largeSDParms.SDLargeCDIMode');
...
%[img, pow, threshData] = vsCDIAdaptiveThreshold_offline(  ac1 , threshParams );

%IQOut = reshape(  double(img)   , Nr, Nc );

IQOut = reshape(  double(ac1)   , Nr, Nc );
