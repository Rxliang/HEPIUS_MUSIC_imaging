function varargout = spectralDoppler_4p9p2_saveiq(varargin)
...
narrowbanding = 0; % may be changed below via evParms.SD
...
%         IQdata = squeeze(varargin{1}); % original scheme with complex IQ input buffer
        ...
        evParmsSD = evalin('base', 'evParms.SD');
        %evParmsSD.narrowbandFlag
        if isfield(evParmsSD, 'narrowbandFlag') & evParmsSD.narrowbandFlag
          narrowbanding=1;
          if isfield(evParmsSD, 'narrowbandIndFlag') & evParmsSD.narrowbandIndFlag
             % these are indices to average to get SD signal
            narrowbandIndFlag = 1;
            narrowbandInd = evParmsSD.narrowbandInd;
          else
            narrowbandIndFlag = 1;
            narrowbanding=0
            keyboard
        ...
        IQdataIn = IQdata; % JSM store full size

        if narrowbanding
            %disp('narrowbanding');
            NumDepthSummation = 1; % do normalization here
            switch ndims(IQdata)
              case 2
                disp('narrow band case, but IQdata data dimension is only 2');
                keyboard
                IQdata = sum(IQdata(1:nRange,1:nPulses),1)/nRange;
                if size(IQdata,1)==1
                  IQdata = IQdata(:).';
                end                
              case 3
                %      IQdata = sum(IQdata(1:nRange,1:nWidth,1:nPulses),[1 2])/ ...
                %         (NumDepthSummation*nWidth);
                IQdata = reshape(IQdata, [nRange*nWidth nPulses]);                
                IQdata = mean(IQdata(evParmsSD.narrowbandInd,:),1);
                %IQdata = squeeze(IQdata).';

            end                                                 
        else % not narrowbanding, individual pixel spectrogram
            if 0
              rangeSubSampleIndex = 1;
              lateralSubSampleIndex = 1;
            end
            % for large SD region, use center
            rangeSubSampleIndex = round(sizeIQ(1)/2);
            NumDepthSummation = 1;

            switch ndims(IQdata)
              case 2
                if sizeIQ(2)==1 % reshape puts pulses 1st
                                % if dim of range gate is 1x1
                    % This is the case for individual pixel spectrogram
                    IQdata = IQdata.';                                        
                else               
              ...
              case 3
                lateralSubSampleIndex = round(sizeIQ(2)/2);
                IQdata = squeeze(...
                    IQdata(rangeSubSampleIndex, lateralSubSampleIndex, 1:nPulses));
                % for live operation, mouse pointed sets the center of the
                % rect ROI, and we get the spectrogram of the central pixel
                % disp('choosing central SD pixel');
                IQdata = IQdata.'; % so that wall filter works on a row
            end
        ...
        %iqdr = wdenoise(real(IQdata), 2);
        %iqdi = wdenoise(imag(IQdata), 2);        
        %figure(10)
        %ax = 1:length(IQdata);
        %plot(ax, real(IQdata), ax,iqdr)
        %IQdata = iqdr+j*iqdi;
        %keyboard
...
if ~disableSpectralDisplay
    ...
    if outStructExists
      assignin('base', 'IQSlideBlock', IQSlideBlock);
      evalin('base', 'sdOut.IQSlideBlocks = cat(2, sdOut.IQSlideBlocks, IQSlideBlock);');
    end
    ...
    outStructExists = evalin('base', 'exist(''sdOut'', ''var'')');
    if outStructExists
      assignin('base', 'Sxx', Sxx);
      evalin('base', 'sdOut.Sxx = cat(2, sdOut.Sxx, Sxx);');
    end
...
hTileExists = evalin('base', 'exist(''hTile'', ''var'')');
