...
Resource.RcvBuffer.rowsPerFrame = 2*184320/128*168; % this should be larger than 128*Receive.endDepth*4 for max depth (doubled for 4X sampling)
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    n=n+1;
    Event(n).info = 'save image';
    Event(n).tx = 0;
    Event(n).rcv = 0;
    Event(n).recon = 0;
    Event(n).process = [procImSave];
