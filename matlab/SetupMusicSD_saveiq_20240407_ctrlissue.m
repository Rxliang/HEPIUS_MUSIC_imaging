...
if isempty(vtData)
  error('MUSIC_DATA not defined in environment');
end
...
evParms.state.semaphoreOpenedState = 1;
PDataIndB = 1;
PDataIndSD = 2;
SDInstanceNo = 1;
evParms.gate.PDataIndSDLarge = PDataIndSD; % PData for SD for this script
evParms.gate.numSD = 0; % so sdlargesavefn thinks it has instance 0+1=1
evParms.state.recording = [];
evParms.state.recording(SDInstanceNo) = 1; % enable recording for SD instance 1
evParms.recording.numIQFrames = 500;

TWIndSD = 2;
evParms.ind.TWIndSD = TWIndSD;
evParms.gate.lateralSubSampleIndex{SDInstanceNo} = 0; % dummy for recording file
evParms.gate.rangeSubSampleIndex{SDInstanceNo} = 0; % dummy for recording file
evParms.gate.PDataIndB = PDataIndB;

if isempty(recordDataFileName)
  recordDataPath = fullfile(vtData, 'tmp');
...
if isnxtfn
  RcvProfile(1).PgaGain = 27;
  %  RcvProfile(1).LnaZinSel = 100; % fixed for NXT
  %RcvProfile(1).LnaGain = 21; % closest to 24 for NXT
  RcvProfile(2).PgaGain = 27; 
  %RcvProfile(2).LnaGain = 21;
  %RcvProfile(2).LnaZinSel = 100;
else    
  RcvProfile(1).PgaGain = 30;
  RcvProfile(1).LnaGain = 24;
  RcvProfile(1).LnaGain = 15; % 15 is lowest, 18 or 24
  RcvProfile(1).LnaZinSel = 28;
  RcvProfile(1).LnaZinSel = 31; % more interference
  RcvProfile(2).PgaGain = 30; 
  RcvProfile(2).LnaGain = 24;
  RcvProfile(2).LnaZinSel = 28;
  RcvProfile(2).LnaZinSel = 31;
end

RcvProfile(2).LnaHPF = 200;
RcvProfile(1).LnaHPF = 200;
  
%RcvProfile(2).LnaZinSel = 0; 
                              %RcvProfile(1).LnaZinSel = 0; 
                              %RcvProfile(1).LnaZinSel = 10; 
                              %RcvProfile(1).LnaZinSel = 15; 
%RcvProfile(1).LnaZinSel = 20; 
%RcvProfile(1).LnaZinSel = 25;  % hard to tell but may be worse than < 25
...
P(1).endDepth = 2*128;          % This should preferrably be a multiple of 128 samples.
...
sgp= spectralDoppler_4p9p2_saveiq('spectrogramParameters',framePeriod,dopSweepTimeIndex);
sweepTime = sgp.THist;
...
noWarn =1;
Trans.frequency = demodfreqcheckfn(10.4167, noWarn);
%Trans.frequency = demodfreqcheckfn(15, noWarn);
Trans.units = 'wavelengths'; % Explicit declaration avoids warning message when selected by default
                             %Trans = computeTrans2_mbc(Trans);  % computeTrans is used for known transducers.
Trans = computeTrans2(Trans);  % computeTrans is used for known transducers.
Trans.maxHighVoltage = 30;    % set a reasonable high voltage limit.

lambda_m = Resource.Parameters.speedOfSound / (Trans.frequency*1e6);
lambda_mm = lambda_m*1e3;
wvl_mm = (Resource.Parameters.speedOfSound/1000)/Trans.frequency;

evParms.Trans.frequency = Trans.frequency;

% Specify PData structure arrays.
% - 2D PData structure
PData(PDataIndB).PDelta = [1.0,0,0.5];
PData(PDataIndB).Size(1,1) = ceil(P(1).endDepth/PData(1).PDelta(3)); % rows
PData(PDataIndB).Size(1,2) = ceil((Trans.numelements*Trans.spacing)/PData(1).PDelta(1)); % cols
PData(PDataIndB).Size(1,3) = 1;      % single image page
PData(PDataIndB).Origin = [-Trans.spacing*(Trans.numelements-1)/2,0,P(1).startDepth]; % x,y,z of upr lft crnr w respect to xducer origin.
% - Doppler PData structure
PData(PDataIndSD).PDelta = [1.0,0,1.0];
PData(PDataIndSD).Size(1,1) = dopRows;     % 4 IQ samples for Doppler
PData(PDataIndSD).Size(1,2) = dopCols;     % single column for Doppler
PData(PDataIndSD).Size(1,3) = 1;             % single image page
                                             %PData(PDataIndSD).Origin = [0,0,ceil(P(2).endDepth/2)];
PData(PDataIndSD).Origin = [ -11.0376   0 39.9254/2];
%PData(PDataIndSD).Origin = [ 0   0 P(2).endDepth];
%PData(PDataIndSD).Region=computeRegions(PData(PDataIndSD));

% - Region Outline for Region Constrain of Doppler Processing
DopRegionLim.x = [-32,32]; % in wavelength
DopRegionLim.y = [4,2*128];
PData(3) = PData(PDataIndB);
...
if 0
PData(4) = PData(1); %PData(PDataIndSD);
PData(4).Region = struct('Shape',struct('Name','Rectangle',...
    'Position',[0,0,DopRegionLim.y(1)],...
    'width', PData(PDataIndSD).Size(1,2),...
    'height',PData(PDataIndSD).Size(1,1)));
end
...
Resource.DisplayWindow(1).Title = 'MUSICSpectralDoppler';
...
Resource.DisplayWindow(1).Position = [250,(ScrnSize(4)-(DwHeight+150))/2, ...  % lower left corner position
                                      DwWidth, DwHeight];
Resource.DisplayWindow(1).ReferencePt = [PData(PDataIndB).Origin(1),0,PData(PDataIndB).Origin(3)];  % 2D imaging is in the X,Z plane
...
TGC(1).Waveform = 1023 * ones(size(TGC(1).Waveform));
...
TGC(2).Waveform = 1023 * ones(size(TGC(2).Waveform));
...
for i = 1:nfrms
    ...
    for j = 1:2:maxPRIs
        ...
        Receive(k+j).Apod(129:192) = 1.0;        
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;
        if j==1, Receive(k+j).callMediaFunc = 1; end
        % -- Doppler.
        %        Receive(k+j+1).Apod(round((33:96)/2)) = 1.0; % start Dop aperture at element 33
        Receive(k+j+1).Apod(129:192) = 1.0; % start Dop aperture at element 33
        Receive(k+j+1).TGC = 2;
...
Process(2).method = 'spectralDoppler_4p9p2_saveiq';
...
SeqControl(5).command = 'setRcvProfile';  % time between synthetic aperture acquisitions
SeqControl(5).argument = 1;


SeqControl(6).command = 'setRcvProfile';  % time between synthetic aperture acquisitions
SeqControl(6).argument = 2;

nsc = 7; % nsc is count for next SeqControl object

if syncFlag
  SeqControl(nsc).command = 'sync';
  SeqControl(nsc).argument = 400000; % us
  seq=[];
...
SeqControl(nsc).argument = 30000; % us
seq.seqControlPreTransferTTNA=nsc;
nsc=nsc+1;
...
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = [3,5]; % set rcv profile in addition JSM
...
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = [4, 6]; % set rcv profile in addition JSM
...
for i = 1:nfrms  % Acquire frames
    ...
    if transferDelayFlag
        % put in a delay for transfer JSM
        Event(n-1).seqControl =  seq.seqControlPreTransferTTNA;
    end
...
if syncFlag
        ...
        Event(n).seqControl = [seq.seqControlIndSync];
        n=n+1;        
end
...
filename = 'MatFiles/L11-4vSpectralDoppler';
save(filename);
...
function rangeChangeCallback(hObject, ~, UIValue )
    ...
    if 0
    cc=1;
    Control(cc).Command = 'set&Run';
    Control(cc).Parameters = {'Parameters',1,'startEvent',1};
    evalin('base','Resource.Parameters.startEvent = 1;');
    %    assignin('base','Control',Control);
    end
    
    cc=1;
    Control(cc) = evalin('base','Control');
    Control(cc).Command = 'update&Run';
    Control(cc).Parameters = {'PData','InterBuffer','ImageBuffer','Receive','TGC','Recon','DisplayWindow'};
...
function dopplerPRFCallback( hObject, ~, UIValue)
    ...
    psched = spectralDoppler_4p9p2_saveiq('restrictAcqPRF',round(framePeriod*1e6),dopPRFnom);
    acqPRF = psched.PRFAcq ;
    dopPRF = psched.PRFDop ;
    ttna_microsec =   psched.PRIAcq_uSec ;
...
%     maxPRIs = psched.pulsesPerFrameAcqMax  ;
    ...
    assignin('base', 'evParms.ev.nDopPRIs', nDopPRIs);
    assignin('base', 'evParms.ev.dopPRF', dopPRF);    
    assignin('base', 'evParms.ev.acqPRF', acqPRF);        
    assignin('base', 'evParms.ev.ttna_microsec', ttna_microsec); 
...
%-UI#5Callback - Sweep Time
    ...
    sgp=spectralDoppler_4p9p2_saveiq('spectrogramParameters',TFrame,1);
...
% Window ButtonDown callback for sample volume positioning.
    ...
    %    persistent hROISD
    %    persistent drawSDROI  % jsm
    
    Trans = evalin('base','Trans');
    Resource = evalin('base','Resource');
    DopRegionLim = evalin('base','DopRegionLim');
    PDataThis = evalin('base','PData(PDataIndSD)');
    ...
    if strcmp(hObject.SelectionType,'normal')
        ...
        if elypHndl == 0
            ...
            indh = find(ishandle(elypHndl));  
            delete(elypHndl(indh));
            elypHndl = 0;
        end

        % jsm need to correct for size of region
        x_mm = x/scaleToWvl; 
        z_mm = z/scaleToWvl;         
        xS_mm = PDataThis.PDelta(1)/scaleToWvl;  
        zS_mm = PDataThis.PDelta(3)/scaleToWvl;         
        zC_wvl = round(PDataThis.Size(1)/2)*PDataThis.PDelta(3); 
        xC_wvl = round(PDataThis.Size(2)/2)*PDataThis.PDelta(1);  
        disp('[xC (mm) zC (mm):]')
        [xC_wvl zC_wvl]*evalin('base', 'wvl_mm')
        %        assignin('base','SVPos',[x,z-2])
        % these are the halfwidths of the region
        xC_mm = xC_wvl/scaleToWvl;
        zC_mm = zC_wvl/scaleToWvl;
        
        xOrigin_mm = (x-xC_wvl)/scaleToWvl; % of PData
        zOrigin_mm = (z-zC_wvl)/scaleToWvl; % of PData        
        
        assignin('base','SVPos',[x-xC_wvl, z-zC_wvl]); %JSM
        %Modify structures for new SV position.
        evalin('base','PData(PDataIndSD).Origin = [SVPos(1),0,SVPos(2)];');
        ...
        if strcmp(VsType,'Matlab')
            ...
            elypHndl(1) = rectangle('position', ...
                                  [x_mm-xS_mm/2  z_mm-zS_mm/2 xS_mm zS_mm], ...
                                    'edgecolor', 'y');            
            elypHndl(2) = rectangle('position', ...
                                  [xOrigin_mm  zOrigin_mm xC_mm*2 zC_mm*2], ...
                                    'edgecolor', 'y');            
if 0
            abox = [x-1,z-1,2,2];
        ...
        %        evalin('base','PData(4).Origin = [SVPos(1),0,SVPos(2)];'); 
        %evalin('base','PData(4).Region=computeRegions(PData(4));');
        
        %        if isempty(drawSDROI)
        % drawROISD = 1;
        % evalin('base','hROISD = drawRegionOutline(1,4,1);');
        %else           
        %  evalin('base','drawRegionOutline(hROISD, 1,4,1);');
        %end
        
          %MRK

        
        %        [x, z-2]*evalin('base', 'wvl_mm')
        disp('[clicked x (wvl) z (wvl):]')        
        [x z]
        disp('[clicked x (mm) z (mm):]')        
        [x z]*evalin('base', 'wvl_mm')        
        
        disp('[x-xC (mm) z-zC (mm):]')        
        [x-xC_wvl, z-zC_wvl]*evalin('base', 'wvl_mm')
        Control = evalin('base','Control');
        Control(1).Command = 'update&Run';
        % JSM added Recon, to prevent weird 
        % dependency on original fake PData(2) origin
        Control(1).Parameters = {'PData', 'Recon'};
        assignin('base','Control', Control);
