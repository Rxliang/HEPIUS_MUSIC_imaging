...
activeEl = zeros(1,128);
activeEl(1:64)=1;
appInd1 = computeMuxAperture(activeEl, Trans)
activeEl = zeros(1,128);
activeEl(65:128)=1;
appInd2 = computeMuxAperture(activeEl, Trans)
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    for j = 1:2:2*P.numRays
        ...
        Receive(k+j).aperture = appInd1;
        Receive(k+j).framenum = i;
        Receive(k+j).acqNum = j;      % two acquisitions per frame
        Receive(k+j+1).Apod(65:112) = 1;
        Receive(k+j+1).aperture = appInd2;
