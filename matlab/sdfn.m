function varargout = sdfn(varargin)
...
if isempty(semaphoreOpenedState)
  ...
  evalin('base', ...
         ['evParms.state.semaphoreOpenedState = ' ...
          num2str(semaphoreOpenedState) ';']);
end
...
if audioOn,
    ...
    if audioDataSave,
        ...
        if invocationCount==FrameCountMax
            ...
            vtData = [getenv('MUSIC_DATA') '/audio/'];
            matOutPath = vtData;
...
function [dataFileName]=saveIQWithTimeStamp(x,NframesIn, instanceNumber,...
  ...
  doInit = ~(size(x,1)==size(X,1) & size(x,2)==size(X,2) & ...
             size(x,3)==size(X,3));
  if doInit,
    disp('sdfn: saveIQWithTimeStamp: doInit');
    pausedMsgPrint=0;
    %initialization
    if nargin<2,
      NframesIn = [];
...
%    recordData.UIModePriority
    ...
    if ~isempty(recordData.stepType) & recordData.stepType(1) == 'w' & ...
          ~recordData.UIModePriority
      % wall track
    ...
    if ~isempty(recordData.stepType) & recordData.stepType(1) == 'd' ...
        & ~recordData.UIModePriority    
    ...
    if evParms.flag.forceRecord | (1 == successFlag)
      ...
      %      disp(['doRecording = ' num2str(doRecording) ' ' ...
      %      'finishRecording = ' num2str(finishRecording)]);
      ...
      if evParms.state.SDOnlyInPlace | ...
        ...
        if evParms.flag.forceRecord | doRecording | finishRecording    
          ...
          nframes = nframes+1;
          nframes = rem(nframes-1,Nframes)+1;
          %          disp(['Frame ' num2str(nframes) ' of ' num2str(Nframes)]);
          
          if nframes==round(nframes/20)*20
            disp(['IQ frame ' num2str(nframes) ' of ' num2str(Nframes)]);
          end
          X(:,:,:,nframes) = single(x);
          dateNum = datenum(now);
		...
		[outFilePre '.fig']);
          ...
          if 0 & saveBufferMat,
            matOutPath = [getenv('MUSIC_DATA') '/nicp/'];
            if ~exist(matOutPath, 'dir')
          ...
          % 20240303: was this |, and was forcing into SDOnly, reprogramming seq
          %        if (doRecording |  forceSwitchtoSDOnly) & ~evParms.state.SDOnlyInPlace               keyboard 
          
          if (doRecording &  forceSwitchtoSDOnly) & ~evParms.state.SDOnlyInPlace   
              % this is broken, seems no way to save without changing seq
              keyboard 
          evParms = evalin('base', 'evParms');
