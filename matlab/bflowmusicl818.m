...
if ~exist(outFilePre, 'dir')
  mkdir(outFilePre)
end

% number of image buffers to allocate to bflow image


Trans.frequency =  10.4167;
Trans.Bandwidth = [4 15];
%Trans.Bandwidth = [7 11];
Resource.Parameters.speedOfSound = 1540;
lambda_mm = 1e3*Resource.Parameters.speedOfSound/(Trans.frequency*1e6);

% carotid
if 0
  P.txFocus_mm = 17; P.endDepth_mm = 20;
  ttna_us = 150/2;
  ttnaFrame_us = 86000;
  TGC.CntrlPts =  [200,375,490,560,630,700,765,830];
  TGC.CntrlPts = [300 511 920.5997 510.7998 1023 1023 709.1999 ...
                  300];
    %[300,511,716,920,1023,1023,300,300]; %[0,138,260,287,385,593,674,810];
else
  P.txFocus_mm = 4; P.endDepth_mm = 6; % P.endDepth_mm = 8; P.endDepth_mm = 12; % 12 to
                                                           % see
                                                           % ulnar nerve 
  ttna_us = 125*4;
  ttnaFrame_us = 77000;
  TGC.CntrlPts = [300 454.5041 474.2336 698.5629 718.2921 533.4214 ...
                553.1509 500];

end
  
P.numDestImBF = 1; % buffers for display of B-flow images
P.startDepth = 5;
P.endDepth = P.endDepth_mm/lambda_mm;   % Acquisition depth in wavelengths
P.txFocus = P.txFocus_mm/lambda_mm; % Initial transmit focus. (adjustable)
P.numRays = 112; % no. of Rays

P.numRepeats = 3; % number of times to repeat each ray
P.numFrames = 10;
...
PData(1).Region = repmat(struct('Shape',struct( ...
                    ...
                    'width',Trans.spacing,...
                    'height',P.endDepth-P.startDepth)),1,Trans.numelements);
% - set position of regions to correspond to beam spacing.
for i = 1:Trans.numelements
    PData(1).Region(i).Shape.Position(1) = (-Trans.numelements/2+0.5 + (i-1))*Trans.spacing;
end
...
roundTripTime_us = 1e6*(2*maxAcqLength*lambda_mm*1e-3)/Resource.Parameters.speedOfSound;

wlsPer128 = 128/(2*4); % wavelengths in 128 samples for 4
                       % samplesPerWave
ReceiveEndDepth =  P.startDepth + wlsPer128*ceil(maxAcqLength/ ...
                                                 wlsPer128);

%Resource.RcvBuffer.rowsPerFrame = 2*184320/128*168; % this should
%be larger than 128*Receive.endDepth*4 for max depth (doubled for
%4X sampling)
Resource.RcvBuffer.rowsPerFrame = 2*4*P.numRays*P.numRepeats*ReceiveEndDepth; % this should be larger than 128*Receive.endDepth*4 for max depth (doubled for 4X sampling)
Resource.RcvBuffer.colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer.numFrames = P.numFrames;
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.

for p = 1:P.numRepeats + P.numDestImBF 
  Resource.ImageBuffer(p).numFrames = 50;
end

Resource.DisplayWindow(1).Title = 'L8-18i rylns 4X sampling at 62.5 MHz';
...
Resource.DisplayWindow(2).Title = 'L8-18i rylns 4X sampling at 62.5 MHz';
Resource.DisplayWindow(2).pdelta = 0.25;
...
%TW = struct('type','parametric',...
 %           'Parameters',[Trans.frequency,.67,2,1]);

TW = struct('type','parametric',...
            'Parameters',[Trans.frequency,.67,8,1]);

depthAxTGC_mm  = linspace(0, P.endDepth_mm, 8);
alpha=0.54;
[atten_dB, comp] = usattenfn(depthAxTGC_mm, TW(1).Parameters(1), alpha);
...
for n = 1:Trans.numelements % 128 transmit events
    % Set transmit Origins to positions of elements.
    TX(n).Origin = [(-Trans.numelements/2+0.5 + (n-1))*Trans.spacing, 0.0, 0.0];
...
% let us instead use default BPF since we are not sampling below
% the actual center freq of txd
...
% removed                        'InputFilter', BPF1, ...

Receive = repmat(struct('Apod', ones(1,Trans.numelements), ...
                        'startDepth', P.startDepth, ...
                        'endDepth', ReceiveEndDepth, ...
                        'TGC', 1, ...
                        'bufnum', 1, ...
                        'framenum', 1, ...
                        'acqNum', 1, ...
                        ...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, ...
                        P.numRepeats*P.numRays*Resource.RcvBuffer(1).numFrames);
% - Set event specific Receive attributes.

frm = 0;
for i = 1:Resource.RcvBuffer(1).numFrames
  rr = (i-1)*P.numRays*P.numRepeats;  
  Receive(rr+1).callMediaFunc = 1;
  ac = 1;
  frm=frm+1;
    for j = 1:P.numRays
      rrr = rr+(j-1)*P.numRepeats;      
      for q = 1:P.numRepeats
        Receive(rrr+q).framenum = i;
        Receive(rrr+q).acqNum = ac;
        ac=ac+1;
      end     
    end
end
...
TGCRange_dB = 40;
%gain_dB = (TGCRange_dB-atten_dB(end))+atten_dB;

gain_dB = (25+atten_dB)*1023/TGCRange_dB;
gain_dB(1:5) = [434.7750 454.5041 474.2336 493.9627 513.6923];
%gain_dB = atten_dB;
...
Recon = repmat(struct('senscutoff', 0.6, ...
               ...
               'rcvBufFrame',-1, ...
               'IntBufDest', [0,0], ...
               'ImgBufDest', [1,-1], ... % imbuf, frame no
               'RINums', 1:P.numRays),1, P.numRepeats);

% Specify Recon structure array.
riInd = [];
for q = 1:P.numRepeats
  Recon(q).ImgBufDest = [q, -1];
  riInd(q,:) =  1+(q-1)*P.numRays+(0:P.numRays-1);
  Recon(q).RINums = riInd(q,:);
end
...
ReconInfo = repmat(struct('mode', 'replaceIntensity', ...  % replace intensity data
                   'txnum', 1, ...
                   'rcvnum', 1, ...
                   'regionnum', 0), 1, P.numRays*P.numRepeats);
% - Set specific ReconInfo attributes.

cnt=0;
for q = 1:P.numRepeats
  ind = q:P.numRepeats:P.numRays*P.numRepeats;
  txnum = num2cell(1:P.numRays);
  thisRiInd = riInd(q,:);
  [ReconInfo(thisRiInd).txnum] = txnum{:};
  rcvnum = num2cell(ind);
  [ReconInfo(thisRiInd).rcvnum] = rcvnum{:};
  regionnum = num2cell(1:P.numRays);
  [ReconInfo(thisRiInd).regionnum] = regionnum{:};
end

BFlowImBufferNo = P.numRepeats + P.numDestImBF;
...
Process(2).Parameters = {'imgbufnum', 4, ...%BFlowImBufferNo,...   % number of buffer to process.
                         'framenum',-1,...   % (-1 => lastFrame)
                         'pdatanum',1,...    % number of PData structure to use
                         'pgain',1.0,...            % pgain is image processing gain
                         'reject',2,...      % reject level
                         ...
                         'displayWindow',2};
procImSaveStart=3;
if 1 % saves 3 frames in sequence
for p = procImSaveStart:procImSaveStart+P.numRepeats-1;
...
for p = procImSaveStart:procImSaveStart+P.numRepeats-1;
  Process(p).classname = 'External';
  Process(p).method = ['bflowextfn'];              
  Process(p).Parameters = {'srcbuffer','image', ... 
                         'srcbufnum', p-procImSaveStart+1, ...
                         'srcframenum', -1, ...
                         'dstbuffer', 'image', ...
                         'dstbufnum', 4, ...% BFlowImBufferNo,...
                         'dstframenum', -1};
end
end
...
SeqControl(1).argument = ttna_us; % roundTripTime_us*1.35;  % 200 usec between ray lines
SeqControl(2).command = 'timeToNextAcq';
% 66100 is okay, but not necessarily allowing for image saving to mem
SeqControl(2).argument = max(45,round(ttnaFrame_us - P.numRepeats*(Trans.numelements-1)*SeqControl(1).argument)); % 22.5 frames per second
...
for i = 1:Resource.RcvBuffer(1).numFrames
  rr = (i-1)*P.numRays*P.numRepeats;
  for j = 1:P.numRays                 % Acquire all ray lines for
                                        % frame
    rrr = rr+(j-1)*P.numRepeats;
    for k = 1:P.numRepeats
      Event(n).info = ['rl: ' num2str(j) ',' num2str(k)];
      Event(n).tx = j;
      Event(n).rcv = rrr+k;
      Event(n).recon = 0;
      Event(n).process = 0;
...
UI(2).Control = {'UserA1','Style','VsSlider','Label',['Range (',AxesUnit,')'],...
                 'SliderMinMaxVal',[40,300,P.endDepth]*wls2mm,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
...
filename = 'gel18rylns256.mat';
outFile = 'gel18rylns256.mat';
save(outFile);
lslrt(outFile);
