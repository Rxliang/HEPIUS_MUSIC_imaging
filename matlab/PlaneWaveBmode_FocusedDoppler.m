...
P.numAvgs = 3;        % number of averages for the Bmode image
P.numPlanes = 25;    % number of planes used to create a Bmode image
P.maxFPS = 100;        % Frame per second for Bmode imaging
P.c = 1540;           % Speed of Sound (m/s)      
P.beta = 4;         % Narrowness of the kaiser window
P.useElem = [1, 64];

%% Plane Wave Doppler User Parameters
dop.saveData = 0; % Set this to 1 to save the generated data
dop.outDir = 'C:\Users\Administrator\Documents\Vantage-4.5.3-2107301223\20231109_PigSurgery';
dop.fname = 'CDI_noavg_arr3_spot2';

dop.PRI = 100;        % Pulse repetition interval (number of images in an ensemble)
...
%% Focused Beam Color Doppler Imaging params
cdi.numLines = 8;
cdi.focalpt = [0,0,10]; %mm
cdi.fnum = 2;
cdi.numAvg = 1;
...
PData(2).Size(2) = cdi.numLines+1;
PData(2).Size(3) = 1;
PData(2).Origin = [-cdi.numLines/2*PData(2).PDelta(1), PData(1).Origin(2), dop.startDepth];

shapeCen = linspace(-cdi.numLines/2*PData(2).PDelta(1),cdi.numLines/2*PData(2).PDelta(1),cdi.numLines);
PData(2).Region = repmat(struct('Shape',struct('Name','Rectangle',...
                                               'Position',[0,PData(1).Origin(2),dop.startDepth],...
                                               'width', PData(2).PDelta(1),...
                                               'height',dop.endDepth-dop.startDepth)),1,cdi.numLines);
for i = 1:cdi.numLines
    PData(2).Region(i).Shape.Position(1) = shapeCen(i);
    PData(2).Region(i).Shape.Position(2) = PData(1).Origin(2);
end
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'peakBLMax',2,...
                   'peakCutOff',0.8,...
                   'aperture',1),1,(P.numPlanes+cdi.numLines));
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'InputFilter', [],...
                        'callMediaFunc', 0,...
                        'aperture',1), 1, Resource.RcvBuffer(1).numFrames*P.numAvgs*P.numPlanes + Resource.RcvBuffer(2).numFrames*(dop.PRI*cdi.numAvg*cdi.numLines+P.numPlanes*P.numAvgs));%(Resource.RcvBuffer(1).numFrames*P.numAvgs*P.numPlanes + Resource.RcvBuffer(2).numFrames*(dop.numAngs*dop.PRI+P.numPlanes*P.numAvgs)+Resource.RcvBuffer(3).numFrames*(cdi.numLines*dop.PRI+P.numPlanes*P.numAvgs))
...
%             Receive(currframe_rcv+(k-1)*P.numAvgs+l).sampleMode = 'NS200BW';
    ...
    for j = 1:cdi.numLines*dop.PRI
        for m = 1:cdi.numAvg
            ...
            Receive(bkgdUSRcv+(j-1)*cdi.numAvg+m).endDepth = dop.endDepth;
...
UI(9).Control = VsButtonGroupControl('LocationCode','UserB7','Title','Doppler Mode',...
...
filename = 'C:\Users\Administrator\Documents\Vantage-4.8.4-2211151000\MatFiles\MUSIC_CDIDoppler.mat';
save(filename);
VSX;
...
%changePRF
dop = evalin('base','dop');
dopProcess = evalin('base','PDIProcess');
...
Control = evalin('base','Control');
Control(1).Command = 'set&Run';
Control(1).Parameters = {'Process',dopProcess,'prf',dop.PRF};
...
Process = evalin('base','Process');
for k = 1:2:length(Process(dopProcess).Parameters)
    if strcmp(Process(dopProcess).Parameters{k},'pwrThreshold'), Process(dopProcess).Parameters{k+1} = UIValue; end
end
