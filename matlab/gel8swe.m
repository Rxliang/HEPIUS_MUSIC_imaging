...
% set speed of sound in m/sec before calling computeTrans
Resource.Parameters.speedOfSound = 1540; 

if usem5
  useFreq_MHz =   3.6765; % 6.9444; % 10.4167; % 12.5;
  startDepth_mm = 0.5;
  endDepth_mm = 35;
else
  useFreq_MHz = 8.9286;
...
Trans.frequency = useFreq_MHz;
lambda_mm = Resource.Parameters.speedOfSound/Trans.frequency*1e-3;

machPush = 0; numPush=3;

if ~machPush
  numPush = 1;
end

zSpeed_mps = 6; % speed of moving source
...
%% System parameters, dangerous so set "filename" here
filenamePre = ('gel18swe');
...
pushCycle  = 750;     % was 500 number of cycles
if usem5
  maxVoltage = 50;
else
  maxVoltage = 35; % be careful with L8-18i as can get hot
end

pushDuration_us = pushCycle / useFreq_MHz;

ttnaAcq_us =  100
ttnaPush_us = 500; % pushDuration_us*1.5;  % 500;
ttnEB_us =  200000;
...
if usem5
  ...
  SWIFocusX = 0;
  SWIFocusZ = 40; % was 50
  SWIROI.width = 80; % was 80;
  SWIROI.height = 120; % was 50;  % [width and height in
                       % wavelength]
else
    % Define ROI for IQ processing, can be changed in GUI
  SWIROI.xzOrigin_mm = [-5; 1.25];
  SWIROI.xzExtent_mm = [10; 8.00];
  SWIFocusXZ_mm = [-4.5; 2.3]; 

%  SWIROI.width = 60; % was 80;
%  SWIROI.height = 50; % was 50;  % [width and height in wavelength]
end

SWIFocusX = SWIFocusXZ_mm(1)/lambda_mm;
SWIFocusZ = SWIFocusXZ_mm(2)/lambda_mm;
 
SWIROI.width = round((SWIROI.xzExtent_mm(1)-SWIROI.xzOrigin_mm(1))/lambda_mm);
SWIROI.height = round((SWIROI.xzExtent_mm(2)-SWIROI.xzOrigin_mm(2))/lambda_mm);
SWIROI.xzOrigin_wvl = SWIROI.xzOrigin_mm/lambda_mm;
...
V64LE = 0;
switch num2str(SysConfig.AcqSlots)
    case '1  0  0  0' % V32 LE or V64
        error('This script does not support V32 LE or V64 configuration!');
    case '1  0  0  1' % V64 LE or V128
        if SysConfig.LEsys
            V64LE = 1;
...
%% Imaging parameters
% if these are fractional, messes with movie size calc in processIQ
P.startDepth = ceil(5*lambda_mm);   % Acquisition depth in wavelengths
%P.endDepth = 160;   % This should preferrably be a multiple of 128
                    % samples.
P.endDepth =  ceil(endDepth_mm/lambda_mm);   % This should preferrably be a multiple of 128 samples.
...
PData(1).Size(2) = ceil((numElementsUsed*Trans.spacing)/PData(1).PDelta(1));
PData(1).Size(3) = 1;      % single image page
PData(1).Origin = [-Trans.spacing*(numElementsUsed-1)/2,0,P.startDepth]; % x,y,z of upper lft crnr.
...
PData(2).Size(2) = ceil((numElementsUsed*Trans.spacing)/PData(2).PDelta(1));

xRectPDataC_wvl = SWIROI.xzOrigin_wvl(1)+SWIROI.width/2;
zRectPDataC_wvl = SWIROI.xzOrigin_wvl(2); %+SWIROI.height/2;

%[SWIROI.xzOrigin_wvl(1),0,SWIROI.xzOrigin_wvl(2)],...

PData(2).Region.Shape = struct(...
    'Name','Rectangle',...
    'Position', [xRectPDataC_wvl 0 zRectPDataC_wvl], ...
    'width', SWIROI.width,...
...
Resource.DisplayWindow(1).Title = filenamePre;
...
% mach excitation, couade 2010, 100us per push, 3 pulses, 1 mm sep

if machPush
  pushSep_mm = 1;
  T_us = 1/Trans.frequency;
  TPulse_us = 100;
  NCycle = ceil(TPulse_us/T_us);
  pushCycle = NCycle;     % mm/mmpersec
  ttnaPushInterval_us =  1e6*(pushSep_mm/(zSpeed_mps*1e3));
else
  numPush = 1;
  pushSep_mm=0;
end
...
if strcmp(SysConfig.Option{1},'HIFU')
...
txApodVec = zeros(1, Trans.numelements);
txApodVec(1:numElementsUsed) = 1;
...
TX = repmat(struct('waveform', 1, ...
    ...
    'Steer', [0.0,0.0], ...
    'Apod', txApodVec, ...
    'Delay', zeros(1,Trans.numelements)), 1, nTX);

if machPush
  zFocus = SWIFocusZ-floor(numPush/2)*pushSep_mm:pushSep_mm: ...
           SWIFocusZ+floor(numPush/2)*pushSep_mm;
else
  zFocus = SWIFocusZ;
end
...
for q = 2:nTX
  ...
  TX(q).waveform = 2;
  TX(q).focus = zFocus(q-1);               % wavelength, can be changed in the GUI
  TX(q).focusX = SWIFocusX;                % can be changed in the GUI
  TX(q).pushElements = pushElements;       % can be changed in the GUI

  
  apodStart = round(SWIFocusX/Trans.spacing) + numElementsUsed/2 - TX(q).pushElements/2;
  apodEnd = apodStart + TX(q).pushElements - 1; 
  
  % Apod = 1 for pushing elements
  TX(q).Apod = zeros(1,Trans.numelements);
  TX(q).Apod(apodStart:apodEnd) = 1;
  TX(q).Delay = computeTXDelays(TX(q));
end
...
maxAcqLength = ceil(sqrt(P.endDepth^2 + ((numElementsUsed-1)*Trans.spacing)^2));
...
if V64LE
    ...
    RcvApod = zeros(1,Trans.numelements);
    RcvApod(1:numElementsUsed) = 1;
end
...
% TCG for longitudinal radial artery
TGC.CntrlPts = [295 845 241 915 565 800 850 950];
...
afterpush = 5;
SeqControl(afterpush).command = 'timeToNextAcq';
SeqControl(afterpush).argument = ttnaPush_us;               % 500usec
% - time between detect acquisitions
SeqControl(6).command = 'timeToNextAcq';
SeqControl(6).argument = ttnaAcq_us;               % 100usec
PRF=6;
% - time between frames
seqControlIndTTNEB = 7;
SeqControl(seqControlIndTTNEB).command = 'timeToNextEB';    % set time between extended bursts
SeqControl(seqControlIndTTNEB).argument = ttnEB_us;            % 200000usec = 200msec (~ 5 fps)
...
for i = 1:SWIFrames
    ...
    for q = 1:numPush
      ...
      Event(n).process = 0;
      if q == numPush
        Event(n).seqControl = [afterpush, seqControlIndTTNEB];
      else
        Event(n).seqControl = [afterpush];
      end      
      n = n+1;
    end
...
ROIpos = [SWIROI.xzOrigin_wvl(1), SWIROI.xzOrigin_wvl(2), ...
          SWIROI.width, SWIROI.height];
...
save(['MatFiles/',filenamePre]);
lslrt(['MatFiles/', filenamePre '.mat']);
% only set this is was success saving
...
P = evalin('base','P');
numElementsUsed = evalin('base', 'numElementsUsed');
P.endDepth = UIValue;
assignin('base','P',P);
...
figPos.VSX = [ 2106 49 450 620]*1.5/1.75;
...
ttnaPush_us = evalin('base', 'ttnaPush_us');
ttnaAcq_us = evalin('base', 'ttnaAcq_us');
ttnEB_us = evalin('base', 'ttnEB_us');

numElementsUsed = evalin('base', 'numElementsUsed');

ROILA = (PData(2).Region.PixelsLA+1);

[ii,jj] = ind2ij(ROILA, PData(2).Size(1));

%PData2Mask = zeros(PData(2).Size(1), PData(2).Size(2));
%PData2Mask(ii,jj)=1;
%PData2Mask(ROILA)=1;

if 0 % original way does not work when SWIROI has width of PData
...
Depth = max(ii)-min(ii)+1;
Width = max(jj)-min(jj)+1;
...
closeFileAfterWrite=1;
fileVersion=2;

datenumStart = datenum(now);
dateStr = datestr(datenumStart, 'yyyymmdd_HHMMSS');

iqFileRe = [outFilePre '_' dateStr '.re'];
iqFileIm = [outFilePre '_' dateStr '.im'];

IQ = IBuffer+j*QBuffer;

if 1
savesweiq(IQ, ...
          PData(2).Region.Shape.Position(1), ... % v2
          PData(2).Region.Shape.Position(3), ... % v2
          SWIROI.width, SWIROI.height, ...
          PData(2).PDelta, PData(2).Origin, ROILA, x, z,...
          TX(2).pushElements, ttnaPush_us, ttnaAcq_us, ttnEB_us, na, Trans.frequency, ...
          iqFileRe, iqFileIm, ...
          datenumStart, closeFileAfterWrite, fileVersion);

B = evalin('base', 'imSet.im(:,:,ind)');

iqFileB = [outFilePre '_' dateStr];
spec.PData = PData(1);
h5File = writebmodeh5fn(spec, B, 'B', iqFileB, 'double');
...
if ishandle(bmodeFigHandle)
    if  isempty(recHandle) || ~ishandle(recHandle)
      ...
      %        recHandle =
      %        rectangle('Position',[x-SWIROI.width/2,z-SWIROI.height/2,SWIROI.width,SWIROI.height]%,'EdgeColor','w','LineWidth',2);
      recHandle = ...
          rectangle('Position',[SWIROI.xzOrigin_wvl(1),...
                                SWIROI.xzOrigin_wvl(2),...
                                SWIROI.width,SWIROI.height]);
      set(recHandle, 'EdgeColor','w','LineWidth',2);
      xFullTransLim_wvl = [-(Trans.spacing*numElementsUsed/2) ...
                           +(Trans.spacing*numElementsUsed/2)];
      recTransFull  = rectangle('Position', ...
        [xFullTransLim_wvl(1),4,Trans.numelements*Trans.spacing,4],...
        'EdgeColor','b','FaceColor','b');
      recTrans  = rectangle('Position', ...
        [x-TX(2).pushElements*Trans.spacing/2,4,TX(2).pushElements*Trans.spacing,4],...
        'EdgeColor','r','FaceColor','r');
      markHandle = plot(x,z,'xr','MarkerFaceColor','r', ...
...
if freeze == 0 && vsExit == 0  % no response if freeze or exit
    ...
    PData(2).Region.Shape.Position = [newX,0,newY];
    PData(2).Region = computeRegions(PData(2));
...
elseif strcmp(replay,'off')
    ...
    %hv2 = evalin('base', 'VInit(2);'); % was 1.6; 
    hv2 = 1.6; 
...
%profNo=1;
%keyboard
%hv1 = evalin('base', 'VInit(1);');
%[result,hv] = setTpcProfileHighVoltage(hv1, profNo);
%set(hv1Handle(2), 'Value',hv1);
...
if mod(pushElements,2) == 0
    focusLimit = [Location.PosForEvenPush(pushElements/2),Location.PosForEvenPush(numElementsUsed-pushElements/2)];
    ...
    focusLimit = [Location.PosForOddPush((pushElements+1)/2),Location.PosForOddPush(numElementsUsed-(pushElements-1)/2)];
