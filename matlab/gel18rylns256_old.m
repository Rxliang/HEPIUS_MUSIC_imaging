...
P.txFocus = 40;  % Initial transmit focus.
...
Resource.System.transmitChannels = 256;
Resource.System.receiveChannels = 256;
...
Resource.RcvBuffer.rowsPerFrame = 4096*R;   % this size allows for maximum range
...
Resource.DisplayWindow.Title = mfile;
...
%for i = 1:128
%  activeEl = zeros(1,128);
%  activeEl(i)=1;
%  appInd = computeMuxAperture(activeEl, Trans);
%end

% - Set event specific TX attributes.
for n = 1:P.numRays   % P.numRays transmit events
    % Set transmit Origins to positions of elements.
    TX(n).Origin = [(-(R/2)+0.5 + (n-1))*Trans.spacing, 0.0, 0.0];
    ...
    app = zeros(1,Trans.numelements);
    app(lft:rt)=1;
    %TX(n).aperture = 1; %computeMuxAperture(app, Trans);
    %min(lft, 65);
    %
%    TX(n).aperture = computeMuxAperture(app, Trans);
...
filename = 'gel18rylns.mat';
outFile = 'gel18rylns.mat';
save(outFile);
lslrt(outFile);
%VSX
...
for n = 1:P.numRays   % P.numRays transmit events
    ...
    TX(n).Apod(lft:rt) = 1.0;
    TX(n).aperture = min(lft, Trans.numelements/2+1);
    TX(n).Delay = computeTXDelays(TX(n));
end
assignin('base','TX', TX);
