...
P.endDepthMm = 40;    % end depth in mm
...
P.maxFPS = 100;        % Frame per second fo    r Bmode imaging
P.c = 1540;           % Speed of Sound (m/s)      
P.beta = 4;         % Narrowness of the kaiser window

P.useElem = [65, 128]; %[1, 64];[129, 192][65,128]
...
dop.endDepthMm = 40;  % mm
dop.vesDepth = 15;
dop.imWidthMm = 9;
dop.cycles = 6;
dop.numAngs = 11; %should be odd to allow for tx at 0 degrees
dop.datcols = 512; %Columns in the spectral doppler array.

dop.vesR = 0.25; %mm
dop.prcPwr = 0;
dop.prcRF = 0;
dop.lastInterFrame = 1;
dop.outDir = '20241107_longclosepig';
dop.saveData = 1;
dop.fname = 'WeakFlowMedlow_canon_pre';
dop.cpl = 70;   
dop.vesAng = 45; %degrees
dop.voltage = 1.6;
%% Initialize spectData
spectData = zeros(dop.PRI,dop.datcols);
...
%% Spectral Doppler params
nDopPRIs = dop.PRI;
dopPRF = dop.PRF;
framePeriod = dop.PRI/dop.PRF;
dopWFCutoffNorm = 0.08;
dopSweepTimeIndex = 5;
baseLineShiftNorm = 0;
SDDynRangeDB = 100;
NWindow = 128;
...
Resource.RcvBuffer(2).numFrames = 4;     % 10 frames for RF data.
Resource.InterBuffer(2).numFrames = 4;  % one intermediate buffer needed.
...
Resource.ImageBuffer(2).numFrames = 4;
...
Resource.ImageBuffer(3).numFrames = 1;
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'TXPD',[],...
                   'peakBLMax',2,...
                   'peakCutOff',0.8),1,(P.numPlanes+dop.numAngs));
...
for i = 1:dop.numAngs
    ...
    TX(lastBmodeTxArr1+i).Apod(P.useElem(1):P.useElem(2)) = 1;
...
%     TX(lastBmodeTxArr1+i).focus = dop.focalDepth;
    ...
    TX(lastBmodeTxArr1+i).peakCutOff = 3.5;
...
% end
%%% NXT Codes
% BPFDoppler.sampleMode = 'NS200BW';
% BPFDoppler.demodFrequency = 10.4;
% BPFDoppler.Bandwidth = [1-0.1/2, 1+0.1/2]*BPFDoppler.demodFrequency; % with 0.3 as 30% Bandwidth
% BPFDoppler = computeRcvFilter(BPFDoppler);
% RcvFilter(1) = BPFDoppler;
%                         'rcvFilter', 1,...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'mode', 1, ...
                        'LowPassCoef', lpfilt,...
                        'callMediaFunc', 0), 1, Resource.RcvBuffer(1).numFrames*P.numAvgs*P.numPlanes + Resource.RcvBuffer(2).numFrames*(dop.numAngs*dop.PRI+P.numPlanes*P.numAvgs));%(Resource.RcvBuffer(1).numFrames*P.numAvgs*P.numPlanes + Resource.RcvBuffer(2).numFrames*(dop.numAngs*dop.PRI+P.numPlanes*P.numAvgs)+Resource.RcvBuffer(3).numFrames*(cdi.numLines*dop.PRI+P.numPlanes*P.numAvgs))
...
Process(6).method = 'spectralDoppler_JP';
...
SeqControl(23).command = 'loopCnt';
SeqControl(23).condition = 'counter1';
SeqControl(23).argument = 1;

SeqControl(24).command = 'loopCnt';
SeqControl(24).condition = 'counter1';
SeqControl(24).argument = 0;

SeqControl(25).command = 'loopTst';
SeqControl(25).condition = 'counter1';
looptstseq = 25;

nsc = 26; % nsc is count of SeqControl objects
...
startDopArr1 = n;
Event(n).info = 'Set loopCnt to 0';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = [24,20];
n = n+1;

spectralev = n;
Event(n).info = 'Set loopCnt to 1';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = 23;
n = n+1;

SeqControl(20).argument = n;
...
Event(n).info = 'loopTst';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = 25;
n = n+1;
...
Event(n).info = 'Reset loopCnt to 0';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
Event(n).seqControl = 24;
n = n+1;
...
SeqControl(25).argument = n;
startSpectral = n;
SeqControl(18).argument = n;
for l = 1:Resource.RcvBuffer(2).numFrames
    currframe = (l-1)*(P.numPlanes*P.numAvgs+dop.numAngs*dop.PRI);
    for j = 1:P.numPlanes
        for k = 1:P.numAvgs
    ...
    for i = 1:dop.PRI
        for j = 1:dop.numAngs
            Event(n).info = 'Spectral Doppler Plane TX';
            Event(n).tx = lastBmodeTxArr1 + j;
            Event(n).rcv = lastBmodeRcvArr1 + currframe + P.numPlanes*P.numAvgs + (i-1)*dop.numAngs + j;
            Event(n).recon = 0;
            Event(n).process = 0;
...
Event(n).info = 'Reset loopCnt to 1';
Event(n).tx = 0;
Event(n).rcv = 0;
Event(n).recon = 0;
Event(n).process = 0;
...
UI(9).Control = VsToggleButtonControl('LocationCode','UserC4','Label','Spectral','Style','VsToggleButton');
UI(9).Callback = @runspectral;

% Save all the structures to a .mat file.
filename = 'C:\Users\Administrator\Documents\VantageNXT-1.0.2\MatFiles\MUSIC_PlaneDoppler.mat';
save(filename);
VSX;
...
return
%DopplerMode
function runspectral(~,~,UIState)
    spectral_ev = evalin('base','spectralev');
    startDop = evalin('base','startDopArr1');

    Control = evalin('base','Control');
    Control(1).Command = 'set&Run';
    togbutpwr = findobj('String','Spectral');
    if UIState
        Control(1).Parameters = {'Parameters',1,'startEvent',spectral_ev};
    elseif togbutpwr.Value == 1
        Control(1).Parameters = {'Parameters',1,'startEvent',startDop};
    else
