%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Last updated 2022-Jun-14 by Jen (joharmon@uw.edu)
%
...
% on Ubuntu 18.04, kernel 4.18.0-25-generic and 4.19.0-041900-generic.
%
...
%
% NOTE: Be careful about changing GUI controls too quickly or before
% imaging sequence starts, may result in an error and will require
% restarting Matlab!
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
opts.save_path = '~/Documents/live_scan/';  % Path for saving data - ideally on SSD for speed
opts.save_RF = 1;                           % Raw RF data - SLOWER if set to 1
opts.save_IQ = 1;                           % Unprocessed IQ ensemble - SLOWER if set to 1
...
opts.colorflow_transmit_freq = 13.8889;               % In MHz
opts.colorflow_pulse_length = 3;                      % In cycles
opts.num_angles = 5;                                  % Number of angles for coherent compounding
opts.filter_type = 'SVD';                             % 'SVD' only for now
opts.filter_thresholds = [40 opts.singleshot_PRI];    % For SVD wall filtering
opts.power_display = [95 120];                        % In dB
opts.velocity_display = [-3 3];                       % In cm/sec
opts.power_processing = 'R1';                         % 'Cleaner' or 'R1'
opts.velocity_power_threshold = 100;                  % For thresholding vessels vs. background

% Live Bmode
opts.num_cine_frames = 50;                            % Number of DisplayWindow frames for cine loop

% Bmode display alongside CF singleshot
opts.bmode_display_dynamic_range = 30;                % Display dynamic range in dB 
...
P.endDepthMm = 15.0;     % endDepth in mm
...
TW(2).Parameters = [opts.colorflow_transmit_freq,.67,2*opts.colorflow_pulse_length,1];
...
% Bmode TGC
TGC(1).CntrlPts = [671.8210 900 1023 1023 1023 1023 1023 1023];
TGC(1).rangeMax = P.endDepth;
TGC(1).Waveform = computeTGCWaveform(TGC(1));

% Doppler TGC
TGC(2).CntrlPts = [809.2390 1023 1023 1023 1023 1023 1023 1023];
...
UI(UIelements).Control = {'UserA1','Style','VsSlider','Label',['Range (',AxesUnit,')'],...
                 'SliderMinMaxVal',[2,P.maxDepthMm,P.endDepthMm]*scaleToWvl,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
UI(UIelements).Callback = text2cell('%RangeChangeCallback');
UIelements = UIelements+1;

% Transmit focus change
UI(UIelements).Control = {'UserB4','Style','VsSlider','Label',['TX Focus (',AxesUnit,')'],...
                 'SliderMinMaxVal',[10,600,P.txFocusMm]*scaleToWvl,'SliderStep',[0.1,0.2],'ValueFormat','%3.0f'};
UI(UIelements).Callback = text2cell('%TxFocusCallback');
UIelements = UIelements+1;

% TX Aperture change
UI(UIelements).Control = {'UserB5','Style','VsSlider','Label','TX Aper',...
                 'SliderMinMaxVal',[1,128,P.numTx],'SliderStep',[1/128,1/32],'ValueFormat','%3.0f'};
UI(UIelements).Callback = text2cell('%ApertureCallback');
UIelements = UIelements+1;

% Peak CutOff
UI(UIelements).Control = {'UserB2','Style','VsSlider','Label','Peak Cutoff',...
                  'SliderMinMaxVal',[0,20.0,TX(1).peakCutOff],...
                  'SliderStep',[0.005,0.020],'ValueFormat','%1.3f'};
UI(UIelements).Callback = text2cell('%PeakCutOffCallback');
UIelements = UIelements+1;

% Max. Burst Length
UI(UIelements).Control = {'UserB1','Style','VsSlider','Label','Max. BL',...
                  'SliderMinMaxVal',[0,20.0,TX(1).peakBLMax],...
                  'SliderStep',[0.005,0.020],'ValueFormat','%1.3f'};
UI(UIelements).Callback = text2cell('%MaxBLCallback');
UIelements = UIelements+1;

%% Additional UI for singleshot
% Singleshot transmit frequency
UI(UIelements).Control = {'UserC7','Style','VsSlider','Label','CF Trans Freq',...
                 'SliderMinMaxVal',[8, 26, opts.colorflow_transmit_freq],...
                 'SliderStep',[0.05, 0.1],'ValueFormat','%1.3f'};
UI(UIelements).Callback = text2cell('%TransmitFrequencyCallback');
UIelements = UIelements+1;

% Singleshot pulse length
UI(UIelements).Control = {'UserC8','Style','VsSlider','Label','CF Cycles',...
                 'SliderMinMaxVal',[1, 6, opts.colorflow_pulse_length],...
                 'SliderStep',[0.2, 0.2],'ValueFormat','%.0f'};
UI(UIelements).Callback = text2cell('%TransmitPulseLengthCallback');
UIelements = UIelements+1;

% Fire singleshot
UI(UIelements).Control = {'UserC2', 'Style', 'VsPushButton',...
                'Label', 'Acquire CF'};
UI(UIelements).Callback = text2cell('%SingleshotActivateCallback');
UIelements = UIelements+1;

% Add a control for wall filter thresholds
UI(UIelements).Control = {'UserC1','Style','VsSlider','Label','SVD Thresh',...
                 'SliderMinMaxVal',[2, opts.singleshot_PRI-1, opts.filter_thresholds(1)],...
                 'SliderStep',[0.01, 0.01],'ValueFormat','%.0f'};
UI(UIelements).Callback = text2cell('%SVDSliderCallback');
UIelements = UIelements+1;

% Let user specify filename
savefile = '';                              % Default to nothing in box
UI(UIelements).Control = {'Style', 'edit',...        % Control to edit input text
                 'String', savefile,...     % Initial value
                 'Position', [165,220,275,22],...
                 'Tag','centerFreq',...
                 'BackgroundColor',[0.9,0.9,0.9],...
                 'Callback',{@filenameCallback}};
UI(UIelements).Callback = {'filenameCallback.m',...
...
UI(UIelements).Control = {'Style', 'text',...
                  'String', 'Filename',...
...
evalin('base','TGC(1).rangeMax = P.endDepth;');
evalin('base','TGC(1).Waveform = computeTGCWaveform(TGC(1));');
evalin('base','TGC(2).rangeMax = P.endDepth;');
evalin('base','TGC(2).Waveform = computeTGCWaveform(TGC(2));');
...
%% CF transmit frequency
%TransmitFrequencyCallback
TW = evalin('base', 'TW');
TW(2).Parameters(1) = UIValue;
assignin('base', 'TW', TW);
Control = evalin('base','Control');
Control.Command = 'update&Run';
Control.Parameters = {'Parameters', 'TW'};
assignin('base','Control', Control);
%TransmitFrequencyCallback

%% CF pulse length
%TransmitPulseLengthCallback
TW = evalin('base', 'TW');
TW(2).Parameters(3) = round(UIValue);
assignin('base', 'TW', TW);
Control = evalin('base','Control');
Control.Command = 'update&Run';
Control.Parameters = {'Parameters', 'TW'};
...
%% Process and display Doppler results
...
% Find Bmode intensity frames to load for image blending
flist = dir([opts.save_path '*' savefile '*Bmode_raw_intensity.mat']);
fileCount = length(flist)-1; % Subtract one to account for file already being saved
bmodeIntSave = sprintf([savefile '_CF%02d_Bmode_raw_intensity.mat'], fileCount);
load(fullfile(opts.save_path, bmodeIntSave));
bmodeBck = 10*log10(imgData(:,:,end)); % Pull last Bmode frame for blending

% Setting up for saving pngs, figs; colorflow
flist = dir([opts.save_path '*' savefile '*Power.fig']);
fileCount = length(flist);
dopPwrFigSave = sprintf([savefile '_CF%02d_Power.fig'], fileCount);
dopPwrPngSave = sprintf([savefile '_CF%02d_Power.png'], fileCount);
flist = dir([opts.save_path '*' savefile '*Power_blend.fig']);
fileCount = length(flist);
dopPwrFigSaveBlend = sprintf([savefile '_CF%02d_Power_blend.fig'], fileCount);
dopPwrPngSaveBlend = sprintf([savefile '_CF%02d_Power_blend.png'], fileCount);
flist = dir([opts.save_path '*' savefile '*Velocity.fig']);
fileCount = length(flist);
...
flist = dir([opts.save_path '*' savefile '*Velocity_blend.fig']);
fileCount = length(flist);
dopVelFigSaveBlend = sprintf([savefile '_CF%02d_Velocity_blend.fig'], fileCount);
dopVelPngSaveBlend = sprintf([savefile '_CF%02d_Velocity_blend.png'], fileCount);
flist = dir([opts.save_path '*' savefile '*Doppler_output.mat']);
fileCount = length(flist);
...
% Display flow power
pwrFig = figure('Units', 'normalized', 'Position', [0.23, 0.0, 0.22, 0.5], ...
...
x = 10*log10(pwr);
a = 20;
c = opts.velocity_power_threshold;
vesselMsk = 1./(1+exp(-a*(x-c)));

% Display velocity
velFig = figure('Units', 'normalized', 'Position', [0.0, 0.0, 0.22, 0.5], ...
...
% Blend flow power and phase images with last Bmode frame
maxInt = max(bmodeBck(:));
scal = [maxInt-opts.bmode_display_dynamic_range, maxInt];
blendParams.mindb = scal(1);
blendParams.compression = opts.bmode_display_dynamic_range;
blendParams.maxAlpha = 0.8;
params.nCbarTicks = 6;
pwr_blend = Blend_Images(bmodeBck, 10*log10(pwr), vesselMsk, blendParams, ...
                          opts.power_display(2), opts.power_display(1), ...
                          'power');
vel_blend = Blend_Images(bmodeBck, medfilt2(velCorr), vesselMsk, blendParams, ...
                         opts.velocity_display(2), opts.velocity_display(1), ...
                         'velocity');
                     
% Display blended velocity image
velFigBlend = figure('Units', 'normalized', 'Position', [0.0, 0.0, 0.22, 0.5], ...
                     'Color', 'k', 'Name', dopVelFigSave);
imagesc(vel_blend); axis image
title('Flow Velocity', 'Color', 'w');
set(gca,'XColor','w','YColor','w');
xtx = get(gca, 'xtick'); ztx = get(gca, 'ytick');
mmx = opts.mmPerPxX_dop .* xtx; mmz = opts.mmPerPxZ_dop .* ztx;
...
set(gcf, 'InvertHardcopy', 'off');
caxis([0 1])
h = colorbar('Color', 'w', 'Ticks', linspace(0, 1, params.nCbarTicks), ...
         'TickLabels', linspace(opts.velocity_display(1), ...
                                opts.velocity_display(2), ...
                                params.nCbarTicks));
ylabel(h, 'cm/sec', 'Color', 'w');
colormap(CFI);
                            
% Display blended power image
pwrFigBlend = figure('Units', 'normalized', 'Position', [0.23, 0.0, 0.22, 0.5], ...
                'Color', 'k', 'Name', dopPwrFigSave);
imagesc(pwr_blend); axis image
title('Flow Power', 'Color', 'w');
set(gca,'XColor','w','YColor','w');
xtx = get(gca, 'xtick'); ztx = get(gca, 'ytick');
mmx = opts.mmPerPxX_dop .* xtx; mmz = opts.mmPerPxZ_dop .* ztx;
...
caxis([0 1])
h = colorbar('Color', 'w', 'Ticks', linspace(0, 1, params.nCbarTicks), ...
         'TickLabels', linspace(opts.power_display(1), ...
                                opts.power_display(2), ...
                                params.nCbarTicks));
ylabel(h, 'dB', 'Color', 'w');
colormap hot
...
if opts.save_images
    ...
    saveas(pwrFigBlend, fullfile(opts.save_path, dopPwrFigSaveBlend));
    saveas(pwrFigBlend, fullfile(opts.save_path, dopPwrPngSaveBlend));
    saveas(velFig, fullfile(opts.save_path, dopVelFigSave));
    saveas(velFig, fullfile(opts.save_path, dopVelPngSave));
    saveas(velFigBlend, fullfile(opts.save_path, dopVelFigSaveBlend));
    saveas(velFigBlend, fullfile(opts.save_path, dopVelPngSaveBlend));
...
%% Save Bmode frame associated with CF acquisition
%BmodeFrameSave
save_Bmode(imgData)
ticS = tic;
...
% Always save raw Bmode intensity to load, blend with Doppler
try
...
%% Reset start event to jump back to Bmode after singleshot
%ResetStartingEvent
reset_start_event()
Control = evalin('base','Control');
Control.Command = 'set&Run';
